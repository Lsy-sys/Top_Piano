module Top_Piano(
    ...
    reg        is_break = 0;
    reg [7:0]  playing_key = 0; // 记录当前正在发声的键值，防止多键冲突
    ...
    always @(posedge clk) begin
        if (!sw0 || mode_change_reset) begin
            ...
            if (ps2_fall) begin
                ...
                key_valid <= 1'b0;
                // 如果 PS2 接收计数器停在中间，且超过 10ms 没信号，强制复位
                if (ps2_cnt != 0) begin
                    if (timeout_cnt >= 20'd1_000_000) begin
                        ps2_cnt <= 0;
                        timeout_cnt <= 0;
                    end else timeout_cnt <= timeout_cnt + 1;
                end
            end
        end
    end
    ...
    always @(posedge clk) begin
        if (!sw1 || !sw0 || mode_change_reset) begin
            beat_cnt <= 0;
            score_ptr <= 0;
        end else begin
        ...
        if (!sw0 || mode_change_reset) begin
            ...
            if (current_key == 8'hF0) begin
                ...
                if (is_break) begin
                    ...
                    is_break <= 1'b0;
                    playing_key <= current_key; // 锁定当前按键
                    case (current_key)
                        // 低音
