    ...
    always @(posedge clk) begin
        if (!sw0 || !sw1 || mode_change_reset) begin
            ...
            if (btn_s4) begin play_en <= 1'b1; song_select <= 2'd1; beat_cnt <= 0; score_ptr <= 0; end
                ...
                // 停止逻辑
                if ((song_select == 2'd1 && score_ptr == 6'd31 && beat_cnt >= 25_000_000) || 
                    (song_select == 2'd2 && score_ptr == 6'd39 && beat_cnt >= 25_000_000)) begin
...
if (song_select == 2'd3) begin
    ...
    always @(posedge clk) begin
        if (!sw0 || mode_change_reset || (sw1 && !play_en)) begin
            ...
            if (current_key == 8'hF0) is_break <= 1'b1;
                ...
                case (current_key)
                    // period_reg = 100MHz / (2 * Frequency)
                    8'h16: begin period_reg <= 191113; display_num <= 1; octave_num <= 1; end // L1
                    ...
                    8'h15: begin period_reg <= 95556;  display_num <= 1; octave_num <= 2; end // M1
                    ...
                    8'h1C: begin period_reg <= 47778;  display_num <= 1; octave_num <= 3; end // H1
    ...
    always @(posedge clk) begin
        ...
        else if (sine_acc >= (period_reg >> 7)) begin
            sine_acc <= 0;
            sine_ptr <= sine_ptr + 1;
        end else sine_acc <= sine_acc + 1;
    end

    // (D) 高频 PWM 调制 (约 97.6kHz)
    reg [9:0] pwm_carrier = 0;
    reg sine_pwm_out;
    always @(posedge clk) begin
        pwm_carrier <= pwm_carrier + 1;
        sine_pwm_out <= (pwm_carrier < {sine_rom[sine_ptr], 2'b00});
    end
