...
begin
        ...
        begin
            ...
            begin
                if (beat_cnt >= current_note_duration) 
                begin 
                    beat_cnt <= 0;
...
always @(posedge clk) begin
        ...
        begin
            ...
            if (ps2_cnt != 0) 
            begin
                if (timeout_cnt >= 20'd1_000_000) 
                begin 
                    ps2_cnt <= 0; timeout_cnt <= 0; 
                end
...
always @(posedge clk) 
begin
    if (!sw0 || mode_change_reset || (sw1 && !play_en)) 
    begin
    ...
    begin
        ...
        else if (current_key == 8'h00) 
        begin 
            period_reg <= 0; display_num <= 0; octave_num <= 0; playing_key <= 0; is_break <= 0;
        end 
        else if (is_break) 
        begin
            if (current_key == playing_key || sw1) 
            begin
...
always @(posedge clk) 
begin
    if (period_reg == 0) begin sq_cnt <= 0; sq_wave <= 0; 
    end
    else if (sq_cnt >= period_reg) 
    
    begin 
        sq_cnt <= 0; sq_wave <= !sq_wave; 
    end
...
always @(posedge clk) begin
    if (period_reg == 0) begin sine_acc <= 0; sine_ptr <= 0; 
    end
    else if (sine_acc >= (period_reg >> 7)) begin sine_acc <= 0; sine_ptr <= sine_ptr + 1; 
    end
...
always @(posedge clk) 
begin 
    pwm_carrier <= pwm_carrier + 1; sine_pwm_out <= (pwm_carrier < {sine_rom[sine_ptr], 2'b00}); 
end
