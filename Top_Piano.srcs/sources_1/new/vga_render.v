module vga_render(
    ...
    always @(posedge vga_vs) begin note_q1 <= display_num; note_q2 <= note_q1; end
    ...
    always @(*) begin
        pix_data = 12'h112;
        if (!video_en) pix_data = 12'h000;
        else begin
            if (v_cnt < 50) pix_data = sw1 ? 12'h0A5 : 12'h555;
            else if (v_cnt >= 300 && v_cnt < 475) begin
                if (h_cnt >= START_X && h_cnt < (START_X + 630)) begin
                    if (is_black_key) pix_data = 12'h000;
                    else begin
                        if (rel_x % KEY_W == 0) pix_data = 12'h444;
                        else if (display_num != 0 && current_key_idx == active_key_index) begin
                            case(octave_num)
                                4'd1: pix_data = 12'hF50; 4'd2: pix_data = 12'h0DF; 4'd3: pix_data = 12'hA5F;
                                default: pix_data = 12'h0DF;
                            endcase
                        end else pix_data = 12'hEEE;
                    end
                end
            end
        ...
        vga_r = pix_data[11:8]; vga_g = pix_data[7:4]; vga_b = pix_data[3:0];
    end
endmodule